...
function tabsPortfolio() {
  ...
  filterBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      // Debug clicked category
      const visibleSpan = btn.querySelector("span:not(.d-none)");
const selectedCategory = visibleSpan ? visibleSpan.textContent.trim() : "";
...
const categories = categoryAttr.split(',').map(cat => cat.trim());
...
const show =
 ...
 document.addEventListener("DOMContentLoaded", () => {
  ...
  // Mostrar todos os projetos no load
 // const projectItems = document.querySelectorAll("[data-filter-item]");
