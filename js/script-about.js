...
// Language Stwich
  ...
  switchLang.addEventListener('change', () => {
    ...
    revealWordsScrollBased();
    revealWordsScrollBasedPT();
  });
...
// Animate Text
function revealWordsScrollBased() {
  const container = document.querySelector("#animated-text");
  if (!container) return;

  const words = container.querySelectorAll("span:not(.lang-en):not(.lang-pt)");
  
  // Reset: remove all existing .visible classes
  words.forEach(word => word.classList.remove("visible"));

  const totalWords = words.length;

  // First word always visible
  if (words[0]) words[0].classList.add("visible");

  window.addEventListener("scroll", () => {
    const scrollTop = window.scrollY;
    const revealCount = Math.floor(scrollTop / 10);

    for (let i = 1; i <= revealCount && i < totalWords; i++) {
  ...
  // Animate Text PT
function revealWordsScrollBasedPT() {
  const container = document.querySelector("#animated-text-pt");
  if (!container) return;
